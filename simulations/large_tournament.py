...
for sim in range(n_sim):
    ...
    if sim == 0:
        ...
        results = group.compete(p_matrix=penny_comp, n_rounds=n_rounds,
                                save_history=True, silent=True)
        results['n_sim'] = sim
    else:
        # Run the tournament again
        result_onesim = group.compete(p_matrix=penny_comp, n_rounds=n_rounds,
                                      save_history=True, silent=True)
        ...
        results.\
            to_pickle(r'simulations/results/Large_Simulation_results_temp.pkl')
