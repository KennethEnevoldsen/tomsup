...
```{r}
est = read_csv("../parameters_estimation_100_vol_set.csv")
...
# p(k)
est %>% 
  ggplot(aes(round, estimated_prob_k, group=round)) +
  geom_boxplot(width = 0.8,fill="steelblue", outlier.shape = NA, color="steelblue") +
  facet_wrap(~k,  labeller = as_labeller(c("0" = "Estimated probability of k=0",  "1"= "Estimated probability of k=1"))) + 
  labs(x="Round", y=" ") + 
  theme_bw() + 
