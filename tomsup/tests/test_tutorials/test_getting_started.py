...
def test_tutorial():
    ...
    results = ts.compete(jung, skinner, p_matrix=penny, n_rounds = 4, save_history=True, verbose = True)
    print(type(results))
    ...
    results = ts.compete(jung, skinner, penny, n_rounds=4, n_sim=2, save_history=True, return_val='df', verbose=False)
    results.head()
    ...
    group.plot_choice(agent0 = "RB", agent1 = "QL", agent = 0, plot_individual_sim=False, show=False)
    ...
    group.plot_history("1-TOM", "2-TOM", agent=1, state="", fun=lambda x: x['internal_states']['own_states']['p_op_mean'][0], show=False)

    df = group.get_results()
    df.loc[(df["agent0"] == "1-TOM") & (df["agent1"] == "2-TOM")]["history_agent1"][1]['internal_states']['opponent_states'][1]['own_states']

    # volatility
    group.plot_history("1-TOM", "2-TOM", agent=1, state="", fun=lambda x: x['internal_states']['opponent_states'][1]['own_states']['param_mean'][0, 0], ylab="Volalitity (log-odds)", show=False)
    # 

    # behav temp 
    group.plot_history("1-TOM", "2-TOM", agent=1, state="", fun=lambda x: x['internal_states']['opponent_states'][1]['own_states']['param_mean'][0, 1], ylab="Behavioral Temperature (log-odds)", show=False)
