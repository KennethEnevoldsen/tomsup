...
def plot_heatmap(df, aggregate_col='payoff_agent',
    ...
    if isinstance(certainty_fun, str):
        if certainty_fun.startswith("mean_ci_"):
            ...
            certainty_fun = add_def_args(mean_confidence_interval,
                                         {'confidence': ci})

    # calc aggregate matrix
    df_mean = df_[[aggregate_col + "0", 'agent0', 'agent1']].\
        groupby(['agent0', 'agent1']).\
        apply(lambda x: aggregate_fun(x)).\
        reset_index()
    df_mean2 = df_[[aggregate_col + "1", 'agent0', 'agent1']].\
        groupby(['agent0', 'agent1']).\
        apply(lambda x: aggregate_fun(x)).\
        reset_index()
    ...
    heat_df = pd.pivot(df_mean, values=aggregate_col,
                       index='agent1',
                       columns="agent0")

    if certainty_fun is not None:
        df_ci = df_[[aggregate_col + "0", 'agent0', 'agent1']].\
            groupby(['agent0', 'agent1']).\
            apply(lambda x: mean_confidence_interval(x)).\
            reset_index()
        df_ci.columns = ['agent0', 'agent1', 'ci']
        df_ci2 = df_[[aggregate_col + "1", 'agent0', 'agent1']].\
            groupby(['agent0', 'agent1']).\
            apply(lambda x: mean_confidence_interval(x)).\
            reset_index()
        ...
        df_ci['ci'] = \
            [f"{round(m, 3)} ({round(sd[0][0], 3)}, {round(sd[1][0], 3)})"
             for m, sd in zip(df_mean[aggregate_col], df_ci['ci'])]
        annot_df = pd.pivot(df_ci, values='ci',
                            index='agent1',
                            columns="agent0")
...
def plot_op_states(df, agent0, agent1, state, level=0, agent=0):
    ...
    """
    plot_history(df, agent0, agent1, state="p_op", agent=agent,
                 fun=lambda x: x['internal_states']['opponent_states']
                                [level]['own_states'][state])