...
def p_opk_approx_fun(prev_p_op_mean, prev_param_var, prev_gradient, level):
    ...
    for level_idx in range(level):
        prev_var_prepped[level_idx] = prev_param_var[level - 1, :].\
            T.dot(prev_gradient[level - 1, :]**2)

    # Equation
    p_opk_approx = (prev_p_op_mean + b * prev_var_prepped**c) /\
        np.sqrt(1 + a * prev_var_prepped**d)
...
def p_k_udpate(prev_p_k, p_opk_approx, op_choice, dilution=None):
    ...
    new_p_k = (
        ...
        (1-op_choice) *
        (prev_p_k * (1-p_opk_approx) / sum(prev_p_k *
                                           (1 - p_opk_approx))))
...
def param_mean_update(prev_p_op_mean, prev_param_mean, prev_gradient, p_k,
                      param_var, op_choice):
    """
...
def expected_payoff_fun(p_op, agent, p_matrix):
    ...
    expected_payoff = (
        ...
        (1 - p_op) * (p_matrix.payoff(1, 0, agent) - p_matrix.payoff(0, 0,
                                                                     agent)))
...
def learning_function(prev_internal_states,
                      params,
                      self_choice,
                      op_choice,
                      level,
    ...
    if level == 0:
        ...
        p_op_var0 = p_op_var0_update(prev_p_op_mean0, prev_p_op_var0,
                                     volatility)
    ...
    # If the (simulated) agent is a k-ToM
        ...
        p_opk_approx = p_opk_approx_fun(prev_p_op_mean, prev_param_var,
                                        prev_gradient, level)
        p_k = p_k_udpate(prev_p_k, p_opk_approx, op_choice, dilution)

        # Update parameter estimates
        param_var = param_var_update(prev_p_op_mean, prev_param_var,
                                     prev_gradient, p_k, volatility, **kwargs)
        param_mean = param_mean_update(prev_p_op_mean, prev_param_mean,
                                       prev_gradient, p_k, param_var,
                                       op_choice)
        ...
        for sim_level in range(level):
            ...
            sim_prev_internal_states = copy.\
                deepcopy(prev_internal_states['opponent_states'][sim_level])
        ...
        # Gather own internal states
        own_states = {'p_k': p_k, 'p_op_mean': p_op_mean,
...
# Full k-ToM Function
def k_tom(prev_internal_states,
          params,
          self_choice,
          op_choice,
          level,
    ...
    if self_choice is not None:
        new_internal_states = learning_function(prev_internal_states,
                                                params,
                                                self_choice,
                                                op_choice,
                                                level,
    ...
    p_self, p_op = decision_function(new_internal_states,
                                     params,
                                     agent,
                                     level,
                                     p_matrix)
...
def init_k_tom(params, level, priors='default'):
    ...
    # If the (simulated) agent is a k-ToM
        ...
        # Gather own internal states
        own_states = {'p_k': p_k, 'p_op_mean': p_op_mean,
                      'param_mean': param_mean, 'param_var': param_var,
                      'gradient': gradient}
